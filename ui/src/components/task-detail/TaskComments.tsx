...
export function TaskComments({ comments, isLoading, error, onRefresh, allFiles = [] }: TaskCommentsProps) {
  return (
    <Stack spacing="0">
      <Stack direction="row" align="center" spacing="1.5" className="mb-2">
        ...
        {comments && (
          <Text variant="dimmed">({comments.length})</Text>
        )}
        <Button
          variant="ghost"
          size="icon-xs"
      ...
      {isLoading ? (
        <Text variant="dimmed" className="pl-0.5">Loading comments...</Text>
      ) : error ? (
        <Text variant="dimmed" className="pl-0.5 text-destructive">{error.message}</Text>
      ) : !comments?.length ? (
        <Text variant="dimmed" className="pl-0.5">No comments yet</Text>
      ) : (
        <div className="rounded-lg border border-border bg-card shadow-island-sm divide-y divide-border">

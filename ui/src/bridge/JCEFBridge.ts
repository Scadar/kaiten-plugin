...
export class JCEFBridge {
  ...
  private retryTimerIds: ReturnType<typeof setTimeout>[] = [];
  private retryCount = 0;
  private static readonly MAX_RETRIES = 50; // 50 × 100 ms = 5 s max
  ...
  private sendReadyHandshake(): void {
    // Stop retrying if bridge has already settled (ready or error).
    if (this.state !== 'initializing') return;

    if (typeof window.__jcef_send__ !== 'function') {
      if (this.retryCount >= JCEFBridge.MAX_RETRIES) {
        this.log('warn', 'Max retries reached waiting for __jcef_send__');
        return;
      }
      this.retryCount++;
      const id = setTimeout(() => this.sendReadyHandshake(), 100);
      this.retryTimerIds.push(id);
      return;
    }
   ...
   */
  dispose(): void {
    // Clear all pending retry timers before changing state so sendReadyHandshake
    // callbacks that fire after dispose() find state !== 'initializing' and bail.
    for (const id of this.retryTimerIds) clearTimeout(id);
    this.retryTimerIds = [];
    this.state = 'error'; // prevents new retries
    this.rpc.cancelAll();
    this.eventBus.clear();
